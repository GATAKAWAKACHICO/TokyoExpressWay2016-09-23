...
public class CarControl : MonoBehaviour
{
    ...
    public void dragToStop()
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            car.wheels[i].applyMotorTorque(0);
        }
    }
    ...
    public void goRevears()
    {
        ...
        directWheelsTorque(car.engine.getWheelsTorque());
    }

    public void goForward()
    {
        setCarStatus(CarStause.Forward);
        sendTorque(car.engine.getWheelsTorque());
    }


    public void sendTorque(float wheelTorque)
    {
        if (car.accessories.tcsSystem != null && car.accessories.tcsSystem.m_TractionControl > 0)
            car.accessories.tcsSystem.wheelsTractionSystem(wheelTorque);
        else
            directWheelsTorque(wheelTorque);
    }

    public void directWheelsTorque(float wheelTorque)
    {
        for (int i = 0; i < car.motorWheelsCount; i++)
        {
            car.motorWheels[i].applyMotorTorque(wheelTorque);
        }
    }
